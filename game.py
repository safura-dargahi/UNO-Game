...
class Card:
    def __init__(self, card_type: Optional[CardType], color: Optional[CardColor]):
        ...
        self.is_wild: bool = self.card_type in (CardType.CARD_WILDCARD, CardType.CARD_PLUS_4) if self.card_type else False
        self.is_special: bool = self.is_wild or (self.card_type in (CardType.CARD_PLUS_2, CardType.CARD_REVERSE, CardType.CARD_SKIP) if self.card_type else False)
        self.color: Optional[CardColor] = None if self.is_wild else color
        
        # Fixed: Allow None card_type for wildcard color changes
        if self.card_type is None and self.color is None:
            raise InvalidCardException("Card must have either a type or a color")

    def __repr__(self) -> str:
        if self.card_type is None and self.color is not None:
            return f'* {self.color.name}'
        
        # card_type should not be None here based on logic
        if self.card_type is None:
            return "Unknown Card"
        
        card_type_name: str = ' '.join(self.card_type.name.split('_')[1:]).replace('PLUS ', '+')
        
        if self.is_wild and self.color is None:
            return card_type_name
        
        if self.color is None:
            return card_type_name
        
        return f'{card_type_name} {self.color.name}'
...
class Deck:
    def draw(self, number: int = 1) -> list[Card]:
        ...
        """
        # Fixed: Actually generate cards instead of returning stack reference
        return [self.stack for _ in range(number)]

    @staticmethod
    def _stack() -> Generator[Card, None, None]:
        # Fixed: Generate non-wild cards only for initial draws
        card_type: CardType = random.choice([*CardType])
        
        # If it's a wild card, give it a random color for deck generation
        if card_type in (CardType.CARD_WILDCARD, CardType.CARD_PLUS_4):
            yield Card(card_type, None)
        else:
...
class Player:
    def __init__(self, name: str = ''):
        self.hand: list[Card] = []
        self.name: str = name
        self.is_computer: bool = 'computer' in self.name.lower() if self.name else False
        ...
        if card.playable(self.last_played_card):
            ...
            if card.is_wild:
                if self.turn.is_computer:
                    ...
                    event = GameEvent(GameEventType.AWAIT_COLOR_INPUT, {'player': self.turn})
                ...
                if card.card_type == CardType.CARD_PLUS_2:
            ...
            raise CardNotPlayableError(f"The player {player.name} cannot play {card}.")
...
class Game(Table):
    def __init__(self, players: list[Player], rules: dict[str, Any]):
        ...
        # Make sure first card is not special
...
class Turn:
    ...
    def most_reasonable_color(self) -> CardColor:
        card_colors: list[CardColor] = [card.color for card in self.hand if card.color is not None]
        if not card_colors:
            return random.choice([*CardColor])
        # Type cast to help type checker understand this is CardColor
        return max(set(card_colors), key=lambda c: card_colors.count(c))
